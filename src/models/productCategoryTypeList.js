...
const list = Tree.select('productCategoryTypeList')

class ProductCategoryTypeList{
	static init(){
		const listRoot = list.get()
		if(is.null(listRoot))
			Tree.set('productCategoryTypeList', listMap)
	}
	static loadList(id, force = true){
		return new Promise((resolve, reject) => {
			const listRoot = list.get('root')
			if(force)
				axios.post('productCategoryType/listAll', {id})
				.then(response => {
	...
	static delete(values){
		return new Promise((resolve, reject) => {
			axios.post('productCategoryType/delete', {...values})
			.then(response => {
				resolve(response)
			})
		})
