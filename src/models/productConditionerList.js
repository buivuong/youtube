...
const list = Tree.select('productConditionerList')

class ProductConditionerList{
	static init(){
		const listRoot = list.get()
		if(is.null(listRoot))
			Tree.set('productConditionerList', listMap)
	}
	static loadList(){
		return new Promise((resolve, reject) => {
			const listRoot = list.get('root')
			if(listRoot.length === 0){
				axios.post('productConditioner/listAll')
				.then(response => {
...
export default ProductConditionerList